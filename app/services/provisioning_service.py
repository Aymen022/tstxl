...
def _provision_instance(
    ...
    """Execute GRANT/REVOKE on a single instance with retry + backoff."""
    for attempt in range(1, retries + 1):
        try:
            ...
            log_with_context(
                logger, "WARNING", f"{action} failed on instance",
                provisioningId=provisioning_id,
                instance=instance_name,
                mail=mail,
                attempt=attempt,
    ...
    # All retries exhausted — raise so caller marks it as failed
    raise RuntimeError(
        f"{action} failed on {instance_name} after {retries} attempts"
    )
...
def process_provisioning(
    ...
    retries: int = config.get("xld_api_retries", 3)
    ...
    for instance_name in instance_names:
        ...
        try:
            ...
            log_with_context(
                logger, "ERROR", f"{action} exhausted retries on instance",
                provisioningId=provisioning_id,
                instance=instance_name,
                mail=mail,
                error=str(e),
